...
/**
 * Receiver for each item's `content` lambda in [AnimatedColumn] and [AnimatedRow].
 *
 * **Layers:** For typical fade + slide on an item, use [animatedItem] with [AnimatedColumn] or [AnimatedRow] configured
 ...
 * track the list's own [com.rend1x.composeanimatedlist.animation.AnimatedItemTransitionSpec] (non-`none`)
 * or placement channel explicitly.
 *
 ...
 * 2. **Progress trio** — Usually derived from the same animatables [AnimatedColumn] / [AnimatedRow] uses for
 *    the item shell (fade, slide, optional height/width). With [com.rend1x.composeanimatedlist.animation.AnimatedItemDefaults.none],
 ...
 *
 * - **[visibilityProgress]** — Completion of **fade and slide** from
 *   [com.rend1x.composeanimatedlist.animation.EnterSpec] / [com.rend1x.composeanimatedlist.animation.ExitSpec]
 *   only. It does **not** reflect the item's clipped height/width; for that use [placementProgress]. It is **1f**
 ...
 * - **[placementProgress]** — Completion of the item's **reserved height** in [AnimatedColumn] or **reserved width**
 *   in [AnimatedRow] when [com.rend1x.composeanimatedlist.animation.PlacementBehavior.Animated] is used (`0f..1f` expand/collapse);
 ...
 *
 * **Present-settle:** After the internal list marks an item as present, [AnimatedColumn] / [AnimatedRow] runs a short
 ...
 *
 * When [AnimatedColumn] / [AnimatedRow] applies a new [items] snapshot or a new [transitionSpec] while this item's
 * shell animation is still running, behavior follows [AnimatedColumn] / [AnimatedRow] KDoc and README **Animation
...
interface AnimatedItemScope {
    ...
    /**
     * Fade/slide completion for the item (`0f..1f`). See class documentation for present-settle and
     * the distinction from [placementProgress].
     */
    val visibilityProgress: Float

    /**
     * Animated main-axis placement completion (`0f..1f`) when placement is animated; otherwise **1f**.
     */
