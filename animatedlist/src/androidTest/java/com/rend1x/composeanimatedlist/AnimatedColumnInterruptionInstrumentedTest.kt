...
class AnimatedColumnInterruptionInstrumentedTest {
    ...
    fun given_none_transition_when_items_change_then_shell_animation_state_is_skipped() {
        var items by mutableStateOf(emptyList<Row>())
        ...
        composeRule.onNodeWithTag("row-a").assertIsDisplayed().assertTextEquals("Visible")
        composeRule.runOnIdle {
        ...
        composeRule.onAllNodesWithTag("row-a").assertCountEquals(0)
        composeRule.runOnIdle {
            assertFalse(listState.isAnimating)
        }
    }
